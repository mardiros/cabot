...
impl Display for CabotError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let description = match self {
            CabotError::SchemeError(scheme) => {
                format!("Unmanaged scheme: {}", scheme)
            }
            CabotError::OpaqueUrlError(err) => {
                format!("Opaque URL Error: {}", err)
            }
            CabotError::HostnameParseError(name) => {
                format!("Invalid hostname: {}", name)
            }
            ...
            CabotError::CertificateError(err) => {
                format!("CertificateError: {}", err)
            }
            // Unexpexcted Error, not used
            CabotError::HttpResponseParseError(_) => {
                format!("Unexpected error")
            }
            CabotError::EncodingError(err) => {
                format!("EncodingError error: {}", err)
            }
        };
